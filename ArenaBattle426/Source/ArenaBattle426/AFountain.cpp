...
void AAFountain::BeginPlay()
{
	...
	//UE_LOG(ArenaBattle426, Warning, TEXT("Actor Name : %s, ID : %d, Location X : %.3f"), *GetName(), id, GetActorLocation().X);
	ABLOG_S(Warning);
	ABLOG(Warning, TEXT("Actor Name : %s, ID : %d, Location X : %.3f"), *GetName(), ID, GetActorLocation().X);
}

void AAFountain::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
	Super::EndPlay(EndPlayReason);
	ABLOG_S(Warning);
}

void AAFountain::PostInitializeComponents()
{
